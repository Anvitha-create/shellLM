...
7. Use only plain ASCII characters in all responses.
...
class LLMProvider:
    ...
    def execute(self, command: str, cwd: str, session_history: list) -> tuple:
        ...
        # If static handler returned nothing, treat command as unhandled
        if result is None:
            handled = False
            static_out = ""
            new_cwd = cwd
        ...
        # Return static response if command was handled
        if handled:
            if self.trace:
                print(
                    f"[TRACE] <- STATIC: {repr(static_out[:80])}"
                )
